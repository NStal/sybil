...
exports._httpGet = (option,callback)->
    ...
    if not proxy
        ...
        req.on "error",(err)->
            ...
            callback err
        req.end()
    else if proxy and (proxy.type is "phttp" or not proxy.type)
        host = proxy.host
        port = proxy.port
        ...
        req.on "error",(err)->
            ...
            callback err
        req.end()
    else if proxy and proxy.type is "http"
        host = proxy.host
        port = proxy.port
        if not host or not port
            callback "invalid proxy option"
        ...
        req.on "error",(err)->
            ...
            callback err
        req.end()
    else if proxy and proxy.type is "https"
        host = proxy.host
        port = proxy.port
        if not host or not port
            callback "invalid proxy option"
        ...
        req.on "error",(err)->
            ...
            callback err
        req.end()
    else if proxy and proxy.type is "socks5"
        host = proxy.host or "localhost"
        port = proxy.port or 1080
        if url.indexOf("https") is 0
            scheme = require "socks5-https-client"
        else if url.indexOf("http") is 0
            scheme = require "socks5-http-client"
        else if url.indexOf("feed://") is 0
            scheme = require "socks5-http-client"
            url = url.replace("feed://","http://")
        else
        ...
        urlObject.method = "GET"
        urlObject.headers = headers
        urlObject.socksHost = host
        urlObject.socksPort = port
        req = scheme.request urlObject,(res)=>
            clearTimeout timer
            if res.headers["location"]
                newUrl = require("url").resolve(url,res.headers["location"])
        ...
        callback new Error "invalid proxy"
