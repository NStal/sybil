...
class SubscribeTester
    ...
    start:()->
        ...
        mc.invoke "detectStream","https://twitter.com/nstalorz/lists/vips",(err,stream)=>
            if err
                throw err
                return
...
#            rl.question "user and password(u:p):",(answer)=>
            console.log {cid:data.candidate.cid,username:"nstalorz",secret:"lenFriedBK201"}
            @inf.messageCenter.invoke "authCandidate",{cid:data.candidate.cid,username:"nstalorz",secret:"lenFriedBK201"},(err)->
                console.error err,"???"
                done()
new SubscribeTester()