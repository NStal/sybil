...
(function() {
  ...
  ImageLoader = (function(_super) {
    ...
    ImageLoader.prototype.stop = function() {
      var aborts, err, idleWorker, index, item, srcs, worker, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1, _ref2, _ref3, _results;
      ...
      for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
        item = _ref[index];
        if (_ref1 = item.src, __indexOf.call(srcs, _ref1) >= 0) {
      ...
      for (_j = 0, _len1 = _ref2.length; _j < _len1; _j++) {
        worker = _ref2[_j];
        if (_ref3 = worker.info.src, __indexOf.call(srcs, _ref3) >= 0) {
          aborts.push(worker.info);
          worker.reset();
          idleWorker.push(worker);
        }
      }
      for (_k = 0, _len2 = aborts.length; _k < _len2; _k++) {
        item = aborts[_k];
        item.callback(err);
      }
      _results = [];
      for (_l = 0, _len3 = idleWorker.length; _l < _len3; _l++) {
        worker = idleWorker[_l];
        _results.push(this._onWorkerIdle(worker));
      }
      return _results;
    };
